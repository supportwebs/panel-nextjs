...
import {getAllProformaInvoices} from "../../services/userService";
...
export default  function Page(){
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const { data: invoicesData } = await getAllProformaInvoices();
  ...
  const handleDeleteInvoice = async (invoiceId) => {
    try {
      ...
      const { data: deleteData } = await axios.post(`http://localhost/admin/api/deleteinvoice?id=${invoiceId}`); // فرض کردیم که آدرس API برای حذف کاربرها `/api/users/:userId` است
  ...
  const updateInvoicesList = async () => {
    try {
      const { data: updatedInvoicesData } = await getAllInvoices();
      setInvoices(updatedInvoicesData.data);
    } catch (err) {
      console.log(err.message);
    }
  };
                        ...
                        <tr key={invoice.id}>
                          ...
                          <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                            <div className="flex">
                              ...
                              <Link href={`/viewinvoice/${invoice.id}`}><i
