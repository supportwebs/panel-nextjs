...
import {getAllTickets} from "../../services/userService";
...
export default  function Page(){
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const { data: ticketsData } = await getAllTickets();
  ...
  const handleDeleteTicket = async (ticketId) => {
    try {
      ...
      const { data: deleteData } = await axios.post(`http://localhost/admin/api/deleteticket?id=${ticketId}`); // فرض کردیم که آدرس API برای حذف کاربرها `/api/users/:userId` است
  ...
  const updateTicketsList = async () => {
    try {
      const { data: updatedTicketsData } = await getAllTickets();
                        ...
                        <tr key={ticket.id}>
                          ...
                          <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                            <div className="flex">
                              ...
                              <Link href={`/viewticket/${ticket.id}`}><i
                                  className="fa-solid fa-eye text-blue-500"></i></Link>
                              <Link href={`/editticket/${ticket.id}`}>
