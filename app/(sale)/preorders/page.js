...
import {getAllPreorders} from "../../services/userService";
...
export default  function Page(){
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const { data: preordersData } = await getAllPreorders();
    ...
    const handleDeletePreorder = async (preorderId) => {
        try {
            ...
            const { data: deleteData } = await axios.post(`http://localhost/admin/api/deletepreorder?id=${preorderId}`); // فرض کردیم که آدرس API برای حذف کاربرها `/api/users/:userId` است
    ...
    const updatePreordersList = async () => {
        try {
            const { data: updatedPreordersData } = await getAllPreorders();
            setPreorders(updatedPreordersData.data);
        } catch (err) {
            console.log(err.message);
        }
    };
                                ...
                                <table className="min-w-full leading-normal">
                                    ...
                                    {preorders.map(preorder => (
                                        ...
                                        <tr key={preorder.id}>
                                            ...
                                            <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                      <span
                          className="relative inline-block px-3 py-1 text-green-900 leading-tight">
                       ...
                       {preorder.servicces}
                                            ...
                                            <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                                <div className="flex">
                                                    ...
                                                    <Link href={`/viewpreorder/${preorder.id}`}><i className="fa-solid fa-eye text-blue-500" ></i></Link>
                                                    <Link href={`/editpreorder/${preorder.id}`}>
                                                    ...
                                                    <button onClick={() => handleDeletePreorder(preorder.id)}>
