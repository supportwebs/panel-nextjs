...
import {getAllContracts, getAllPreorders} from "../../services/userService";
...
export default  function Page(){
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const { data: usersData } = await getAllContracts();
    ...
    const handleDeleteUser = async (userId) => {
        try {
            ...
            const { data: deleteData } = await axios.post(`http://localhost/admin/api/deleteuser?id=${userId}`); // فرض کردیم که آدرس API برای حذف کاربرها `/api/users/:userId` است
    ...
    const updateUsersList = async () => {
        try {
            const { data: updatedUsersData } = await getAllUsers();
