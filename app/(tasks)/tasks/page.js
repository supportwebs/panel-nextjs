...
export default  function Page(){
  ...
  const handleDeleteTask = async (taskId) => {
    try {
      ...
      const { data: deleteData } = await axios.post(`http://localhost/admin/api/deletetask?id=${taskId}`); // فرض کردیم که آدرس API برای حذف کاربرها `/api/users/:userId` است
  ...
  const updateTasksList = async () => {
    try {
      const { data: updatedTasksData } = await getAllTasks();
