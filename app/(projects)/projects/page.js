...
export default  function Page(){
  ...
  const handleDeleteProject = async (projectId) => {
    try {
      ...
      const { data: deleteData } = await axios.post(`http://localhost/admin/api/deleteproject?id=${projectId}`); // فرض کردیم که آدرس API برای حذف کاربرها `/api/users/:userId` است
  ...
  const updateProjectsList = async () => {
    try {
      const { data: updatedProjectsData } = await getAllProjects();
                        ...
                        <tr key={project.id}>
                          ...
                          <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                            <div className="flex">
                              ...
                              <Link href={`/viewproject/${project.id}`}><i
