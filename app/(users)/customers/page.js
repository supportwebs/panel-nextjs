...
import {getAllCustomerdatas, getAllUsers} from "../../services/userService";
...
export default  function Page(){
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const { data: customerdatasData } = await getAllCustomerdatas();
        setCustomerdatas(customerdatasData.data);
      } catch (err) {
  ...
  const handleDeleteCustomerdata = async (customerdataId) => {
    try {

      // ارسال درخواست DELETE به سرور برای حذف کاربر
      const { data: deleteData } = await axios.post(`http://localhost/admin/api/deletecustomerdata?id=${customerdataId}`); // فرض کردیم که آدرس API برای حذف کاربرها `/api/users/:userId` است
    console.log(deleteData)
      //  بروزرسانی وضعیت یا رفرش صفحه بعد از حذف کاربر
      updateCustomerdatasList();
    } catch (error) {
      console.error("Error deleting customerdata:", error.message);
    }
  };


  const updateCustomerdatasList = async () => {
    try {
      const { data: updatedCustomerdatasData } = await getAllCustomerdatas();
      setCustomerdatas(updatedCustomerdatasData.data);
    } catch (err) {
      console.log(err.message);
    }
  };
                  ...
                  <table className="min-w-full leading-normal">
                    ...
                    {customerdatas.map(customerd => (
                        ...
                        <tr key={customerd.id}>
                          ...
                          <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                            <p className="text-gray-900 whitespace-no-wrap">
                              {customerd.hosting_period}
                            </p>
                          </td>
                          <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                            <p className="text-gray-900 whitespace-no-wrap">
                              {customerd.bdateperiod}
                            </p>
                          </td>
                          <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                            <p className="text-gray-900 whitespace-no-wrap">
                              {customerd.support_period}
                            </p>
                          </td>
                          <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                            <p className="text-gray-900 whitespace-no-wrap">
                              {customerd.bdatehosting}
                            </p>
                          </td>
                          ...
                          <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                            <div className="flex">
                              ...
                              <Link href={`/viewcustomerdata/${customerd.id}`}><i
                                  className="fa-solid fa-eye text-blue-500"></i></Link>
                              <Link href={`/editcustomerdata/${customerd.id}`}>
                              ...
                              <button onClick={() => handleDeleteCustomerdata(customerd.id)}>
                                <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 0 24 24" width="24px"
                                     fill="#FF0000">
                                  <path d="M0 0h24v24H0V0z" fill="none"/>
                                  <path
