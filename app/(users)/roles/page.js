...
export default  function Page(){
  ...
  const handleDeleteRole = async (roleId) => {
    try {
      ...
      const { data: deleteData } = await axios.post(`http://localhost/admin/api/deleterole?id=${roleId}`); // فرض کردیم که آدرس API برای حذف کاربرها `/api/users/:userId` است
  ...
  const updateRolesList = async () => {
    try {
      const { data: updatedRolesData } = await getAllRoles();
                  ...
                  <table className="min-w-full leading-normal">
                    ...
                    {/*     className="mx-auto object-cover rounded-full h-10 w-10 "/>*/}
                      ...
                      <div className="flex">
                        ...
                        <Link href={`/viewrole/${role.id}`}><i className="fa-solid fa-eye text-blue-500" ></i></Link>
